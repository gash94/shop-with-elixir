const save = (key, value) => {
  ...
  const atTheOldToad = {
    refresh() {
      ...
      // usuniecie wszystkeigo i nowy init dla selecta
      comboinit.innerHTML = "";
      ...
      let options = load("list-potions").map((obj) => obj.name);
    ...
    removePotion(potionName) {
      ...
      if (potionIndex === -1) {
        ...
        function removePot(potion, animate) {
          const listItem = document.querySelector(`[data-id="${potion}"]`);
          listItem.classList.add(`${animate}`);
          setTimeout(() => {
            listItem.remove();
          }, 810);
    ...
    updatePotionName(oldName, newName) {
      ...
      if (oldName === newName) {
        return Notiflix.Notify.warning(`Nazwa eliksiru nie może być taka sama!`);
      }
      const potionIndex = potions.findIndex((object) => object.name === oldName);
      ...
      function updateNamePot(oldName, newName) {
        const listItem = document.querySelector(`[data-id="${oldName}"] h5`);
        listItem.classList.add("animate__animated");
  ...
  btnUpdate.addEventListener("click", () => {
    ...
    atTheOldToad.updatePotionName(oldNameIn.value.trim(), newNameIn.value.trim());
    oldNameIn.value = "";
    newNameIn.value = "";
  });
